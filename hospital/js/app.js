...
* @Last Modified time: 2018-09-10 11:56:57
*/
var hospital_app = angular.module( 'app-hospital', ['ngRoute', 'ngAnimate',  
...
hospital_app.config([ '$routeProvider', '$locationProvider', function( $routeProvider, $locationProvider ) {
	$routeProvider
	.when('/hospital-dashboard', {
		templateUrl: 'hospital/pages/home.html',
		controller: 'hospital_appController0'
	})
	.when('/hospital-profile', {
		templateUrl: 'hospital/pages/profile.html',
		controller: 'hospital_appControllerProfile'
	})
	.when('/hospital-settings', {
		templateUrl: 'hospital/pages/settings.html',
		controller: 'hospital_appControllerSettings'
	}) 
	.when('/hospital-confirmation', {
		templateUrl: 'hospital/pages/confirmation.html',
		controller: 'hospital_appControllerConfirmation'
	}) 
	.when('/hospital-user-existing', {
		templateUrl: 'hospital/pages/user-existing.html',
		controller: 'hospital_appControllerUserExisting'
	}) 
	.when('/hospital-report-disease', {
		templateUrl: 'hospital/pages/report-disease.html',
		controller: 'hospital_appControllerReportDisease'
	}) 
	.when('/hospital-reported-disease', {
		templateUrl: 'hospital/pages/reported-disease.html',
		controller: 'hospital_appControllerReportedDisease'
	}) 

	.when('/hospital-view-disease', {
		templateUrl: 'hospital/pages/view-disease.html',
		controller: 'hospital_appControllerViewDisease'
	}) 


	.when('/hospital-report-file', {
		templateUrl: 'hospital/pages/report-file.html',
		controller: 'hospital_appControllerReportFile'
	}) 
	.when('/hospital-reported-file', {
		templateUrl: 'hospital/pages/reported-file.html',
		controller: 'hospital_appControllerReportedFile'
	}) 
...
//hospital_appControllerDoctorSingleView
hospital_app.run( function($rootScope, cfpLoadingBar,  $location, $timeout, $window) {
...
hospital_app.filter('active', [ function() {
...
// hospital_app.run(function(editableOptions, editableThemes) {
...
hospital_app .directive('inputType', ['$compile', function ($compile) {
	return {
		restrict: 'E', 
		scope: true, 
...
// hospital_app.directive('showTab',
// function () {
// return {
// link: function (scope, element, attrs) {
...
hospital_app.service('myservice', function() {
	this.value = null;
	this.name = null;
});

hospital_app.config(function(toastrConfig) {
...
hospital_app.filter('orderObjectBy', function(){
...
// hospital_app.config(function(paginationTemplateProvider) {
// 	paginationTemplateProvider.setPath('assets/js/dirPagination.tpl.html');
// });

hospital_app.directive('parsleyValidateInput', function($timeout) {
	return {
		link: function(scope, element, attrs) {
...
hospital_app.directive('pdf', function() {
	return {
		restrict: 'E',
		link: function(scope, element, attrs) {
...
hospital_app.factory('serverService', function($http) {
	return {
...
hospital_app.run(['editableOptions', function(editableOptions) {
  editableOptions.theme = 'bs3'; // bootstrap3 theme. Can be also 'bs2', 'default'
}]);

hospital_app.filter('ageFilter', function() {
...
hospital_app.controller( 'SystemControllerBoady',  function($timeout,$route, $location, $scope, $http, $filter , $window, myservice, toastr, cfpLoadingBar , serverService){
	$scope.exit =  function(){
		$window.location.href = 'exit';
	};
    
     $scope.openurlnew1 = () => {	
		$location.path('doctor-report-disease');
        // $window.location.href = 'doctor-report-disease';
        console.log("HAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAIIIIIIIIIIIIIIIII");
	};
    $scope.openurlnew2 = () => {	
		$location.path('doctor-report-file');
        // $window.location.href = 'doctor-report-disease';
        console.log("HAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAIIIIIIIIIIIIIIIII");
	};
...
hospital_app.controller( 'hospital_appControllerProfile', function($timeout, $location, $scope, $http, $window, toastr, myservice, cfpLoadingBar , serverService){
...
hospital_app.controller( 'hospital_appControllerSettings', function($timeout, $scope, $http, $location, $filter, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
...
$scope.$watch( 'login.repassword', function(newdata) {
	...
	if ( a && b)
		if (a != b) {
			...
			$scope.hospitalLogin.repassword.$invalid = true;
			$scope.hospitalLogin.$invalid = true;
		}
	});
...
$scope.hospitalLoginSubmit = function () {
	...
	serverService.post(data, $scope).then(function(response) {  
		...
		if(success == 2){
			...
			$scope.hospitalLogin.password.$invalid = true;
			$scope.hospitalLogin.$invalid = true;
		...
		if(success == 21){
			...
			$scope.hospitalLogin.newpassword.$invalid = true;
			$scope.hospitalLogin.$invalid = true;
...
hospital_app.controller( 'hospital_appControllerConfirmation', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	$scope.users = [];
...
hospital_app.controller( 'hospital_appControllerUserExisting', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	$scope.users = [];
...
hospital_app.controller( 'hospital_appControllerReportDisease', function($timeout, $scope, $http, $window, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	$scope.diagnosis = [];
	$scope.disease = [];
	$scope.profile = [];
...
hospital_app.controller( 'hospital_appControllerReportedDisease', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	$scope.diseases = [];
	$scope.diseasesBase = [];
	...
	$scope.viewMe = (disease ) => {
		...
		$location.path('hospital-view-disease');
	};
...
hospital_app.controller( 'hospital_appControllerViewDisease', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	...
	if( ! $scope.disease ){
		...
		$location.path('hospital-reported-disease');
	}
...
hospital_app.controller( 'hospital_appControllerReportFile', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
...
hospital_app.controller( 'hospital_appControllerReportedFile', function($timeout, $scope, $http, $location, $window, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
...
hospital_app.controller( 'hospital_appController0', function($timeout, $scope, $http, $location, $filter,$sce, myservice, toastr, cfpLoadingBar, serverService){
	doCheckUser($scope, $http);
	$timeout( function(){ 
	...
	try {
		...
		if(response.success == -1 ){
			...
			$scope.$parent.authentication = {username : loname,
				lockscreen: 'hospital/pages/lockscreen.html',
				isLock: true,
				password: null,
				invalidPassword: false,
