...
/**
 * @Author: indran
 * @Date:   2018-09-04 08:30:00
 * @Last Modified by:   indran
 * @Last Modified time: 2018-09-10 12:04:35
 */
...
// $image = imagecreatefrompng($path);
	...
	function checkUser ( $type ){  
		...
		function setProfile($name , $mobile, $email  , $nname, $nmobile, $bed, $htype, $area, $city,	$district, $pin, $longitude , $latitude,   $user_type ) {
			...
			try{
				...
				$array =  array( 
					...
					'hos_area' => $area, 
					'hos_city' => $city, 
					'hos_district' => $district, 
					'hos_pin' => $pin, 
					'hos_longitude' => $longitude, 
					'hos_latitude' => $latitude 
				);


				$result = selectFromTable (' * ', 'doc_hospital  ',  ' use_id = ' . decrypt($_SESSION[SYSTEM_NAME.'userid0']) . ' '  , $a );
				if( isset($result[0])) {

					$result  = updateTable (' doc_hospital   ', $array,  ' use_id = ' . decrypt($_SESSION[SYSTEM_NAME.'userid0']) . ' AND hos_delete = 0 '  , $a ); 

				} else {

					$array['use_id'] =   decrypt($_SESSION[SYSTEM_NAME.'userid0']) ;
					$result  = insertInToTable (' doc_hospital   ', $array, $a ); 
		...
		function getProfile( $type) {
			...
			try { 
				$result  = selectFromTable (' * ', ' `doc_user` u LEFT JOIN doc_hospital  d ON d.use_id = u.use_id   ',  '  u.use_id = ' . decrypt($_SESSION[SYSTEM_NAME.'userid0']) . ' AND u.use_delete = 0 AND u.use_login = 1  ' , $a ); 
				if( isset($result[0])) {
		...
		function updateDp ($data, $type) {
			$done = false;
			$path = null;
			$sitedirectory = FILES . 'hospital/images';
			global $a;
