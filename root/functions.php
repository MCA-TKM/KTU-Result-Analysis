...
/**
 * @Author: indran
 * @Date:   2018-09-01 01:00:19
 * @Last Modified by:   indran
 * @Last Modified time: 2018-09-01 13:48:03
 */
?> 
<?php
...
function auth_login() {
	...
	if( ! isset( $_SESSION[ SYSTEM_NAME . 'userid'] )   ) {
		...
		switch ($current_now) {
			...
			// case DOCTOR :
			// setLocation( PATH . 'doctor-login');
			// break;
			// case HOSPITAL :
			// setLocation( PATH . 'hospital-login');
			// break;

			default:
			setLocation(  PATH.'user-login' );
			break;
		}
	...
	if( decrypt($_SESSION[ SYSTEM_NAME . 'type']) == 'doctor' && dirname($_SERVER['SCRIPT_NAME']) . '/' !=  DIRECTORY_DOCTOR ) 
		$flag = false;  

	if( decrypt($_SESSION[ SYSTEM_NAME . 'type']) == 'hospital' && dirname($_SERVER['SCRIPT_NAME']) . '/' !=  DIRECTORY_HOSPITAL ) 
		$flag = false;  
...
function auth_use() {
	...
	if( isset( $_SESSION[ SYSTEM_NAME . 'userid'] )   ) {  
		...
		switch (doctor_type()) { 
			...
			case 'doctor':
			setLocation(DIRECTORY_DOCTOR);
			break; 

			case 'hospital':
			setLocation(DIRECTORY_HOSPITAL);
			break; 
